...
LONG pids[MAX_PARENT_PIDS] = {0};
PFLT_FILTER FilterHandle = NULL;
PFLT_PORT port = NULL;
PFLT_PORT ClientPort = NULL;
PMESSAGE FltMessage = NULL;
void PcreateProcessNotifyRoutine(
    HANDLE ParentId,
    HANDLE ProcessId,
    BOOLEAN Create)
{
    int index = findFirstsZero(pids, MAX_PARENT_PIDS); // if we have space
    if (index != -1)
    {
    ...
    PULONG ReturnOutputBufferLength)
{
    ...
    RtlZeroMemory(FltMessage, sizeof(MESSAGE));
    if (FltMessage == NULL)
    {
        KdPrint(("Failed to allocate memory for FltMessage\n"));
        // Handle the error appropriately
        return STATUS_SUCCESS; // or another appropriate error code
    }
    ...
    PVOID *CompletionContext)
{
    ...
    PReply reply_struct;
    LONG process = FltGetRequestorProcessId(Data);
    if (process == pids[0])
    {
        return FLT_PREOP_SUCCESS_NO_CALLBACK;
    }
    if (
        ...
        !FLT_IS_IRP_OPERATION(Data) || isIgnored(fullPath, FltMessage))
    {
        return FLT_PREOP_SUCCESS_NO_CALLBACK;
    }
    ...
    {
        if (STATUS_OBJECT_NAME_NOT_FOUND == FileExists(FilterHandle, FltObjects->Instance, fullPath->Buffer))
        {
    ...
    FLT_POST_OPERATION_FLAGS Flags)
{
    ...
    {
        PReply reply_struct = sendMessage(FltObjects->Filter, ClientPort, 1, fullPath);
    ...
    PUNICODE_STRING RegistryPath)
{
    ...
    FltMessage = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(MESSAGE), 'MGT1');
    return status;
}
